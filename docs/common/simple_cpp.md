...
[Address Sanitizer(ASan)](https://github.com/google/sanitizers/wiki/AddressSanitizer) 可以帮助我们定位内存泄漏问题。你通过在编译选项中指定 `-fsanitize=address` 启用 ASan。运行你的程序，如果存在内存泄漏，ASan 会报告可能存在内存泄漏的分配点，如下所示：
